...
public class Tpcc extends STMService {
	...
	class MonitorThread extends Thread {
		public void run() {
			...
			while (count < 10) {
				...
				System.out.format("%5d  %5d  %4.2f %5d %6d\n",
	...
	protected void orderStatus(ClientRequest cRequest, int count, boolean retry) {
		int success = 0;
		...
		TpccWarehouse warehouse = ((TpccWarehouse) stmInstance.open(myid,
				...
				this.stmInstance.OBJECT_READ_MODE, retry, stmInstance.getTransactionId()));
	...
	protected void delivery(ClientRequest cRequest, int count, boolean retry) {
		int success = 0;
		RequestId requestId = cRequest.getRequestId();
		...
		while( xretry == true )
		{
			...
			TpccWarehouse warehouse = ((TpccWarehouse) stmInstance.Xopen(myid,
				...
				this.stmInstance.OBJECT_WRITE_MODE, retry, stmInstance.getTransactionId()));
			if (warehouse == null)
			{
				xretry = true;
				continue;
	...
	protected void stockLevel(ClientRequest cRequest, int count, boolean retry) {
		int success = 0;
		int i = 0;
		RequestId requestId = cRequest.getRequestId();
		final String myid = "w_"
					...
					if (j < NUM_ORDERS_PER_D) {
						...
						TpccOrderline orderline = ((TpccOrderline) stmInstance
								.open(olmyid, this.stmInstance.TX_READ_MODE,
										...
										retry, stmInstance.getTransactionId()));
					}
					j += 1;
				}
			}
	...
	protected void newOrder(ClientRequest cRequest, int count, boolean retry) {
	...
	protected void payment(ClientRequest cRequest, int count, boolean retry) {
		int success = 0;
		RequestId requestId = cRequest.getRequestId();
